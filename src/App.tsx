...
const loadBests = (): Bests => {
  const out: Bests = { easy: 0, normal: 0, hard: 0 };
...
const SpeedDots = ({ n }: { n: number }) => (
  <span className="flex items-center gap-1" aria-hidden>
    {[1, 2, 3].map((i) => (
      <span key={i} className={`h-1.5 w-1.5 rounded-full ${i <= n ? 'bg-firefly-400' : 'bg-moss-300/15'}`} />
    ))}
  </span>
);
        ...
        <header className="mb-5 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-mist-500 sm:text-xs">بازی کلاسیک مار در جنگلِ شب‌تاب‌ها</p>
            </div>
          </div>
          <button
                  ...
                  return (
                    ...
                    >
                      <span className="flex items-center gap-3">
                        <span className={`grid h-4 w-4 place-items-center rounded-full border-2 ${active ? 'border-moss-300' : 'border-mist-500/50'}`}>
                          {active && <span className="h-2 w-2 rounded-full bg-moss-300" />}
                        </span>
                        <span>
                          <span className={`block text-sm font-black ${active ? 'text-moss-200' : 'text-mist-100'}`}>{d.label}</span>
              ...
              <ul className="flex flex-col gap-1.5">
                {DIFF_ORDER.map((id) => {
                  ...
                  return (
                    ...
                    >
                      <span className="font-medium">{DIFFICULTIES[id].label}</span>
            ...
            <section className="rounded-xl border border-moss-300/10 bg-pine-900/80 p-4">
              ...
              <ul className="flex flex-col gap-2.5 text-xs text-mist-400">
                ...
                <li className="mt-1 flex items-start gap-2 rounded-lg bg-pine-800/70 p-2.5 leading-5">
                  ...
                  <span>روی موبایل: روی زمین بازی <b className="text-mist-100">بکشید (سُویپ)</b> یا از دکمه‌های جهت زیر صفحه استفاده کنید.</span>
                </li>
              </ul>
            </section>
          </aside>
